...
export function AdminHeaderPagesTab({ showToast }: { showToast: (msg: string) => void }) {
  ...
  const handleAddTicket = async () => {
    if (!newTicket.title || !newTicket.date || !newTicket.price) {
      showToast("Please fill all required ticket fields.");
      return;
    }
    const token = localStorage.getItem("adminToken");
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                {[0, 1, 2, 3, 4].map((idx) => {
                  ...
                  const isVid = imgUrl.match(/\.(mp4|webm|ogg)/i);
                  return (
        ...
        {activeTag === "galleryH" && (
          <div className="space-y-8 max-w-4xl mx-auto">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
              ...
              <button onClick={() => window.location.href = '/admin/gallery-images'} className="bg-black text-white px-6 py-2 rounded-xl font-bold shadow-lg hover:scale-105 transition-transform w-full md:w-auto text-center">
                View Images
              </button>
            </div>
